...
function ActionStack(type, num_at_once, timebetween){
	...
	var time = (typeof timebetween === "undefined")?"adapt":timebetween;
	...
	this.activate = function(){
		...
		var curtime;
		if(time == "adapt"){
			curtime = 200/stack.length;
		}else curtime = time;
		
		var getnum = (time=="adapt")?num+Math.round(stack.length/10):num;
		var temp = 	stack.splice(get,num);
		setTimeout(function(){
...
function koch_crystal(){

	function establishShape(points){
	...
	var stack = new ActionStack("fifo", 1);
	var canv = document.getElementById("koch");
...
//				]
	...
	for(var i = 0;i<shape.length;i+=2){
		if(shape[i] > 1 || shape[i+1] > 1) throw new Error("Please use Cosin and sin as shape");
		sidelengths[sidelengths.length] = Math.sqrt(Math.pow(shape[i]-shape[(i+2)%shape.length],2) + Math.pow(shape[i+1]-shape[(i+3)%shape.length],2));
	...
	//Cetting up center points and center distances
	var cangles = [];
	var cdist = [];
	var cbot = 0;
	...
	while(i > 10){
		i = nm - nm*2/num_sides;
		nm = i*porp;
		if(i > bot*radius) throw new Error("This shape will grow bigger, not smaller" + i +", " + bot*radius);
		numit++;
	}
	var that = this;
